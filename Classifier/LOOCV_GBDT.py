...
for train, test in loo.split(X):
    params = {'loss': 'deviance', 'learning_rate': 0.1,
          'n_estimators': 1400, 'subsample': 1.0, 'criterion':'friedman_mse', 
...
yscore_sum.to_csv('yscore_GBDT.csv')
ytest_sum = pd.DataFrame(data=y_sparse)
ytest_sum.to_csv('ytest_GBDT.csv')
fpr, tpr, _ = roc_curve(y_sparse[:,0], y_score[:,0])
auc_score=result[6]
lw=2
plt.plot(fpr, tpr, color='darkorange',
lw=lw, label='GBDT ROC (area = %0.4f%%)' % auc_score)
