...
data_1,mask,H=elasticNet(shu,label)#引用降维elasticNet
...
for train, test in loo.split(X):
    cv_clf = LogisticRegression(penalty='l2', dual=False, tol=0.0001, C=1.0, fit_intercept=True, intercept_scaling=1, class_weight=None, random_state=None, solver='liblinear', max_iter=10, multi_class='ovr', verbose=0, warm_start=False, n_jobs=1)
    X_train=X[train]
    y_train=y[train] 
    X_test=X[test]
    y_test=y[test]
...
lw=lw, label='LR ROC (area = %0.4f%%)' % auc_score)
