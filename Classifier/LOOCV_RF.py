...
data=pd.read_csv('ECM_train.csv',header=0)
...
for train, test in loo.split(X):
    ...
    cv_clf = RandomForestClassifier(n_estimators=30, criterion='gini', max_depth=3, 
...
yscore_sum.to_csv('RF_train.csv')
ytest_sum = pd.DataFrame(data=y_sparse)
ytest_sum.to_csv('RF_train.csv')
fpr, tpr, _ = roc_curve(y_sparse[:,0], y_score[:,0])
auc_score=result[6]
lw=2
...
data_csv.to_csv('result_RF_train.csv')
