...
data=pd.read_csv('SMOTE_ECMDA_test.csv',header=0)
...
data_1,mask,H=lassodimension(shu,label)
X=data_1
y=label
y[y==2]=0
...
yscore_sum.to_csv('yscore_LASSO.csv')
ytest_sum = pd.DataFrame(data=y_sparse)
ytest_sum.to_csv('ytest_LASSO.csv')
fpr, tpr, _ = roc_curve(y_sparse[:,0], y_score[:,0])
auc_score=result[6]
lw=2
plt.plot(fpr, tpr, color='darkorange',
lw=lw, label='LASSO ROC (area = %0.4f%%)' % auc_score)
...
data_csv.to_csv('result_LASSO.csv')
