...
yscore_sum.to_csv('yscore_LR_selection.csv')
ytest_sum = pd.DataFrame(data=y_sparse)
ytest_sum.to_csv('ytest_LR_selection.csv')
fpr, tpr, _ = roc_curve(y_sparse[:,0], y_score[:,0])
auc_score=result[6]
lw=2
plt.plot(fpr, tpr, color='darkorange',
lw=lw, label='LR_selection ROC (area = %0.4f%%)' % auc_score)
...
data_csv.to_csv('result_LR_selection.csv')
